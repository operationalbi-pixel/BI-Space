...
public class MainActivity extends ComponentActivity {
    ...
    private void configureWebView() {
        ...
        // Use the normal HTTP cache while staying network-first for updated deployments.
    ...
    private void injectWebViewEnhancements() {
        String script = "(function(){" +
                ...
                "if(host&&!document.getElementById('biSpaceNativeBell')){var bell=document.createElement('button');bell.id='biSpaceNativeBell';bell.type='button';bell.title='Notifikasi dan update';bell.setAttribute('aria-label','Buka notifikasi BI-Space');" +
                "bell.style.cssText='position:relative;display:grid;place-items:center;flex:0 0 32px;width:32px;height:32px;padding:0;border:0;border-radius:10px;background:transparent;color:#74696c;box-shadow:none;cursor:pointer;';bell.innerHTML='<svg viewBox=\"0 0 24 24\" width=\"18\" height=\"18\" aria-hidden=\"true\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.7\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9\"/><path d=\"M10 21h4\"/></svg>';" +
                "bell.onmouseenter=function(){bell.style.background='#f7eef0';bell.style.color='#9f172b'};bell.onmouseleave=function(){bell.style.background='transparent';bell.style.color='#74696c'};" +
                "var unread=Number(BI_SPACE_ANDROID.getUnreadCount()||0);if(unread){var badge=document.createElement('span');badge.id='biSpaceNativeBadge';badge.textContent=unread>99?'99+':String(unread);badge.style.cssText='position:absolute;right:-3px;top:-4px;min-width:15px;height:15px;padding:0 3px;border:2px solid #fff;border-radius:9px;background:#b91e37;color:#fff;font:700 8px/11px sans-serif;box-sizing:border-box';bell.appendChild(badge);}bell.onclick=function(){BI_SPACE_ANDROID.openNotificationCenter()};if(host.classList&&host.classList.contains('top-user')){host.style.gridTemplateColumns=window.innerWidth<=650?'minmax(0,auto) 32px 34px':'36px minmax(0,auto) 32px 36px';host.style.gap='7px';host.insertBefore(bell,host.lastElementChild)}else host.insertBefore(bell,host.lastElementChild);}}catch(e){}" +
